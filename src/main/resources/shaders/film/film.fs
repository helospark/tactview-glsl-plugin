...
{ 
    ...
    col += texture2D(inputImage, vec2(X1, Y1))*0.1;
    col += texture2D(inputImage, vec2(X2, Y2))*0.1;
    col += texture2D(inputImage, vec2(X1, Y2))*0.1;
    col += texture2D(inputImage, vec2(X2, Y1))*0.1;
    
    col += texture2D(inputImage, vec2(invX1, invY1))*0.15;
    col += texture2D(inputImage, vec2(invX2, invY2))*0.15;
    col += texture2D(inputImage, vec2(invX1, invY2))*0.15;
    col += texture2D(inputImage, vec2(invX2, invY1))*0.15;
...
void main(void)
{
    ...
    inputDistort.r = texture2D(inputImage,rCoords).r;
    inputDistort.g = texture2D(inputImage,gCoords).g;
    inputDistort.b = texture2D(inputImage,bCoords).b;
    ...
    gl_FragData[0] = vec4(inputDistort.r,inputDistort.g,inputDistort.b,1.0);
    ...
    gl_FragData[0] = mix(gl_FragData[0], vec4(0.0), gradient);
    ...
    vec3 col = gl_FragData[0].rgb;
    // vec3 col = texture2D(inputImage, texCoord).rgb;
    ...
    // gl_FragData[0] = vec4(vec3(vUv.y), 1.0);
}